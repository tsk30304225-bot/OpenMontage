...
**Atelier implements the visual direction contract; it does not replace it.** For explainers, `visual_direction` (WHAT changes, at which narration words) and the alignment-resolved `visual_timeline` (WHEN) are authoritative inputs to the bespoke composition, which only decides HOW it looks. A visual model without a generic renderer stays in the plan with `renderer: "bespoke"` — never empty `visual_models` because a renderer is missing. Read `skills/core/visual-direction.md` (sections 3 and 5b) before `bespoke-composition.md`; `video_compose` refuses atelier renders whose events are not implemented, and `direction_qa` traces them. Current checkout skills and schemas win over any global prompt or older capability note.
